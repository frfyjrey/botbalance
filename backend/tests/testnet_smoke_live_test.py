...
def test_comprehensive_smoke_create_and_cancel_order():
    ...
    if not actions:
        pytest.skip("No rebalance actions needed - portfolio already balanced")
    ...
    assert orders_created > 0, f"No orders created: {execute_data}"
...
def test_direct_exchange_place_and_cancel_order():
    ...
    try:
        ...
        if usdt_balance < Decimal("10"):
            pytest.skip(f"Insufficient USDT balance: {usdt_balance} (need at least 10)")
        ...
        # Use minimum possible quote amount based on exchange info
        try:
            info = asyncio.run(adapter._get_exchange_info_cached(symbol))
            min_notional = info["min_notional"]
            quote_amount = min_notional * Decimal(
                "2.5"
            )  # Well above minimum to handle rounding
        except Exception:
            # Fallback if exchange info fails
            quote_amount = Decimal("12.0")  # Well above typical $5 minimum
        ...
        pytest.skip(f"Order rejected by exchange: {e}")

    except ExchangeError as e:
        print(f"⚠️  Exchange error (might be expected in testnet): {e}")
        pytest.skip(f"Exchange communication error: {e}")
